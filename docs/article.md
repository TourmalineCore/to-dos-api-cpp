...
В проекте есть `docker-compose.yml`, который описывает конфигурацию для запуска 4-х сервисов:
- `to-dos-api-cpp-db`
- `to-dos-api-cpp-pgadmin`
- `to-dos-api-cpp`
- `to-dos-api-cpp-karate-tests`

Все они объединены в одну общую сеть, что позволяет сервисам обращаться друг к другу по имени.
