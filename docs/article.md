...
Помимо [Drogon](https://drogon.org/), рассматривали [Oat++](https://oatpp.io/) и  [userver](https://userver.tech/). [Oat++](https://oatpp.io/) показался нам более сложным и многословным по сравнению с [Drogon](https://drogon.org/). С [ [userver](https://userver.tech/)](https:// [userver](https://userver.tech/).tech/) ситуация другая, фреймворк отсутствует в `conan-center-index` (хранилище пакетов [Conan](https://conan.io/)), а его установка подразумевает самостоятельную сборку из репозитория разработчиков по их собственному шаблону, что не подходит нам.
...
unit/                   # Unit тесты
e2e/                    # E2E тесты
test_package/           # Служебный conanfilefile для проверки собранного Conan-пакета
...
### [Alembic](https://alembic.sqlalchemy.org/en/latest/) и миграции
...
- Установка Python и пакетов [Alembic](https://alembic.sqlalchemy.org/en/latest/), psycopg2-binary и sqlalchemy-utils
- Инициализация [Alembic](https://alembic.sqlalchemy.org/en/latest/) командой `alembic init migrations`
...
Мы сконфигурировали `CMakeLists.txt` так, чтобы в дальнейшем оставалось только подключать библиотеки, установленные [Conan](https://conan.io/), добавив несколько строк, которые можно уточнить на странице любого пакета из `conan-center-index` [(например, [Drogon](https://drogon.org/))](https://conan.io/center/recipes/drogon?version=1.9.13).
